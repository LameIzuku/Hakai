...
def via_clear(x,y,net,margin=0.02):
    ...
    for it in items:
        ...
        if it[0]=="seg":
            ...
            if net2==net: continue
            if not lays: continue      # paste-only aperture pads: no copper
...
plans=[
 [("seg",FC,155.50,102.25,155.45,102.25),("seg",FC,155.45,102.25,155.45,100.05),
  ("via",155.45,100.05)],
 [("seg",FC,155.50,102.25,155.45,102.25),("seg",FC,155.45,102.25,155.45,100.00),
  ("via",155.45,100.00)],
]
def waived(wr):
    # A8 NC pad pair: fully inside the U4 courtyard -> real rule is 0.072
    it=wr[2]
    return (isinstance(it,tuple) and it[0]=="box" and abs(it[1]-155.75)<0.01
            and abs(it[2]-101.75)<0.01 and wr[0]>=0.072)
done=False
for pi,plan in enumerate(plans):
    ok=True
    for step in plan:
        if step[0]=="via": wr=via_clear(step[1],step[2],"GND",margin=0.01)
        else: wr=seg_clear(step[1],step[2],step[3],step[4],step[5],0.075,"GND",margin=0.01)
        if wr is not None and not waived(wr):
            print(f"plan{pi} {step} blocked: {str(wr[2])[:55]} d={wr[0]:.3f}<{wr[1]:.3f}")
            ok=False; break
    if ok:
        for step in plan:
            if step[0]=="via": add_via(step[1],step[2],"GND")
            else: add_seg(step[2],step[3],step[4],step[5],step[1],"GND",0.10)
        print(f"EP tie: plan{pi} committed"); done=True; break
if done:
    board.Save(PCB); print("saved")
else:
    print("EP tie: ALL plans blocked")
