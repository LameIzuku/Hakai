...
ROOT_UUID = "df71f9f2-97ab-4d7b-a476-8fc54408afff"  # stable: keeps .kicad_pro sheet list valid across regenerations
PROJECT = "hakai_mouse_v6"
# package-relative: the generator always writes the project folder above tools/
...
def make_libs(sh):
    ...
    # AO3401A SOT-23 (1=G 2=S 3=D) -- P-channel, high-side battery-divider
    # switch (v5.2): fully enhanced at Vgs = -VBAT (-3.0..-4.2V)
    pfet = LibPart("AO3401A", "Q", 7.62, "P-MOSFET SOT-23: 1=G 2=S 3=D. High-side battery-divider switch")
    pfet.pin("1", "G", 'L', 0)
    pfet.pin("3", "D", 'R', 0)
    pfet.pin("2", "S", 'R', 1)
    sh.register(pfet)
...
def build():
    ...
    # ---- titles / notes -------------------------------------------------
    sh.text("HAKAI WIRELESS GAMING MOUSE - REV 6.0", 20.32, 15.24, 3.81)
    sh.text("nRF52833 (integrated 2.4GHz radio) + PAW3311 + BQ24074 + 5x Kailh + scroll encoder | SINGLE 1.9V SYSTEM RAIL", 20.32, 21.59, 1.778)
    ...
    sh.text("v5.2: C36 10uF ceramic at BAT pins - BQ24074 Table 7-1 mandatory bypass (battery is on a cable).", 20.32, 186.5)
    sh.text("v5.2: divider gated HIGH-SIDE (Q3 P-FET + R25 pull-up): VBAT_EN low -> Q3 off, ADC node held at GND by R9.", 20.32, 243.05)
    sh.text("v5.3: R25 = 47k (Q1 worst-case IDSS 5uA x 47k = 0.24V << Q3 Vgs(th) min 0.5V - margin doubled).", 20.32, 246.86)
    sh.text("v5.3: sensor board features EXACT per PixArt Table 9: full aperture cutout + optical-axis notch,", 302.26, 228.6)
    sh.text("R0.90x2.00 LED stadium, 2x lens guide-post holes; D2 pads per Table 9 (custom fp, verify vs LED P/N).", 302.26, 232.41)
    sh.text("True zero standby drain; AIN0 can never float above VDD (old low-side gate leaked ~11uA via the ESD clamp).", 20.32, 246.86)
    ...
    # v5.2: mandatory BAT bypass (BQ24074 datasheet Table 7-1: 4.7-47uF ceramic
    # from BAT to VSS) - the battery hangs on a cable, so the CV loop needs a
    # local low-ESR cap at the BAT pins
    c36 = sh.place("C", "C36", "10uF", 116.84, 111.76, footprint=C_FP6)
    sh.connect(c36, {"1": (P, "VBAT"), "2": (P, "GND")})
    ...
    # v5.2: HIGH-SIDE gated divider. Q3 (P-FET) switches VBAT into R8, so with
    # VBAT_EN low the ADC node is held at GND by R9 - it can never float above
    # VDD (the old low-side Q1 let AIN0 rise to VBAT through R8 and leak ~11uA
    # through the nRF ESD clamp in the dominant sleep state). Off-state drain
    # is now FET leakage only (true zero).
    q3 = sh.place("AO3401A", "Q3", "AO3401A", 88.9, 205.74,
                  footprint="Package_TO_SOT_SMD:SOT-23")
    sh.connect(q3, {"1": (LBL, "VBAT_SW_G"), "2": (P, "VBAT"), "3": (LBL, "VBAT_SW")})
    # v5.3: 47k (was 100k) - Q1 off-state leakage x pull-up must stay well
    # under Q3's Vgs(th) min (0.5V): 5uA worst-case x 47k = 0.24V. On-state
    # gate current 4.2V/47k = 89uA only while sampling.
    r25 = sh.place("R", "R25", "47k", 109.22, 205.74, footprint=R_FP)
    sh.connect(r25, {"1": (P, "VBAT"), "2": (LBL, "VBAT_SW_G")})
    r8 = sh.place("R", "R8", "200k", 63.5, 205.74, footprint=R_FP)
    sh.connect(r8, {"1": (LBL, "VBAT_SW"), "2": (LBL, "VBAT_SENSE")})
    r9 = sh.place("R", "R9", "100k", 63.5, 226.06, footprint=R_FP)
    sh.connect(r9, {"1": (LBL, "VBAT_SENSE"), "2": (P, "GND")})
    q1 = sh.place("AO3400A", "Q1", "AO3400A", 88.9, 231.14,
                  footprint="Package_TO_SOT_SMD:SOT-23")
    sh.connect(q1, {"1": (LBL, "VBAT_EN"), "3": (LBL, "VBAT_SW_G"), "2": (P, "GND")})
    ...
    sh.connect(u4, {
        ...
        # right side GPIO
        # v6.0 PIN SWAPS: the aQFN west face could not physically escape all
        # six signals on 2 layers (0.25 mm channels, vias don't fit between
        # stub lines). BTN_RIGHT P0.05->P1.02, BTN_SIDE_BACK P0.06->P1.04,
        # VBAT_EN P0.03->P1.00 moved to free east/south balls. Firmware only.
        "A12": (LBL, "VBAT_SENSE"), "B13": (NC,),
        "J1": (LBL, "BTN_LEFT"), "K2": (NC,),
        "L1": (NC,), "M2": (LBL, "BTN_SIDE_FWD"),
        ...
        "B9": (NC,), "A8": (NC,), "AD22": (LBL, "VBAT_EN"), "Y23": (NC,),
        "W24": (LBL, "BTN_RIGHT"), "B15": (NC,), "U24": (LBL, "BTN_SIDE_BACK"),
        "A16": (NC,), "R24": (NC,), "P23": (NC,), "P2": (NC,), "R1": (NC,),
    })
    ...
    # v5.3: custom pads per PixArt Table 9 "LED Soldering Pad" (flank the
    # R0.90x2.00 stadium cutout); generic 1206 pads would violate the
    # 0.1mm copper-to-edge clearance against the full-size cutout
    d2 = sh.place("LED", "D2", "IR LED (SMT / L0AJ-LSG1 set)", 330.2, 149.86,
                  footprint="hakai_fp:IRLED_L0AJ_3311")
    ...
    # v5.1: middle click is a 5th Kailh GM 8.0 (same tactility as L/R clicks)
    sw5 = sh.place("SW_Kailh_GM8", "SW5", "Kailh GM 8.0", 76.2, 266.7,
                   footprint="hakai_fp:SW_Kailh_GM8_D2F")
    sh.connect(sw5, {"1": (P, "GND"), "2": (LBL, "ENC_SW"), "3": (NC,)})
    sh.text("Wheel: EC1 = rotation (left bay wall, D-socket); SW5 = middle click (Kailh, right wall).", 20.32, 257.81)
    sh.text("Wheel / axle / cradle are shell parts, not PCB items.", 20.32, 261.62)
...
def main():
    ...
    sh = build()
    sch = sh.emit("HAKAI Wireless Gaming Mouse - Rev 6.0")
    ...
    # never clobber an existing project file - it accumulates board setup /
    # DRC severity config that KiCad saves after the first open
    pro_path = os.path.join(OUT_DIR, PROJECT + ".kicad_pro")
