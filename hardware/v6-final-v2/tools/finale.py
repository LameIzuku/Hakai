...
# rip BTN_SIDE_BACK's misrouted east portion (blocked AD22 channel, hit XL2 web)
_rm=0
for _t in list(board.GetTracks()):
    if _t.GetNetname()!="BTN_SIDE_BACK": continue
    if _t.GetClass()=="PCB_VIA":
        _p=_t.GetPosition()
        if TOMM(_p.x)>=152.45: board.Remove(_t); _rm+=1
    else:
        _s,_e=_t.GetStart(),_t.GetEnd()
        if TOMM(_s.x)>=152.45 and TOMM(_e.x)>=152.45:
            board.Remove(_t); _rm+=1
print(f"BS-BACK east rip: {_rm}", flush=True)
board.Save(PCB)
board=pcbnew.LoadBoard(PCB)
netobj.clear()
for fp in board.GetFootprints():
...
SPANS=[
    ("VBAT_EN",      (159.25,108.25),(132.00,133.51),(100,95,166,150)),
    ("BTN_SIDE_BACK",(160.25,106.25),(152.40,104.706),(142,98,166,126)),
]
results={}
