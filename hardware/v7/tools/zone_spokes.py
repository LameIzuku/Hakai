...
# ---- discover isolated GND pad-tongues and route each to a mainland via ----
filler0=pcbnew.ZONE_FILLER(board); filler0.Fill(board.Zones())
...
for fp in board.GetFootprints():
    for p in fp.Pads():
        if p.GetNetname()=="GND":
            q=p.GetPosition(); gpads.append((fp.GetReference()+"-"+p.GetNumber(),TOMM(q.x),TOMM(q.y)))
SPANS=[]
for z in board.Zones():
    for lid in z.GetLayerSet().Seq():
        ...
        for a,oi in areas[1:]:
            if any(polys.Contains(pcbnew.VECTOR2I(int(vx*1e6),int(vy*1e6)),oi) for vx,vy in gvias):
                continue
            inpads=[(nm,px,py) for nm,px,py in gpads
                    if polys.Contains(pcbnew.VECTOR2I(int(px*1e6),int(py*1e6)),oi)]
            if not inpads: continue
            nm,px,py=inpads[0]
            pool=[(vx,vy) for vx,vy in gvias
                  if not polys.Contains(pcbnew.VECTOR2I(int(vx*1e6),int(vy*1e6)),oi)]
            pool+= [(qx,qy) for _,qx,qy in gpads
                    if not polys.Contains(pcbnew.VECTOR2I(int(qx*1e6),int(qy*1e6)),oi)]
            cands=sorted(((abs(vx-px)+abs(vy-py),vx,vy) for vx,vy in pool))[:6]
            for _,vx,vy in cands:
                x0b,y0b=min(px,vx)-6,min(py,vy)-6
                x1b,y1b=max(px,vx)+6,max(py,vy)+6
                SPANS.append(("GND",(px,py),(vx,vy),(x0b,y0b,x1b,y1b)))
            print(f"  isolated {nm} @({px:.1f},{py:.1f}): {len(cands)} targets", flush=True)
print(f"isolated pad-tongues: {len(SPANS)}", flush=True)
results={}
done_src=set()
for net,src,dst,bbox in SPANS:
    if src in done_src: continue
    if route_span(net,src,dst,bbox,0.10):
        done_src.add(src); results[net]=True
