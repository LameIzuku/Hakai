...
# --- surgical pre-clean of known-bad items from earlier passes ---------------
gnd_net=None
for _fp in board.GetFootprints():
    for _p in _fp.Pads():
        if _p.GetNetname()=="GND": gnd_net=_p.GetNet(); break
    if gnd_net: break
removed_v=removed_t=0
for t in list(board.GetTracks()):
    if t.GetNetname()!="GND": continue
    if t.GetClass()=="PCB_VIA":
        p=t.GetPosition()
        if in_poly_box(TOMM(p.x),TOMM(p.y)):        # vias drilled through ANT1 copper
            board.Remove(t); removed_v+=1
    else:
        s,e=t.GetStart(),t.GetEnd()
        for pt in (s,e):
            if (abs(TOMM(pt.x)-155.50)<0.2 and abs(TOMM(pt.y)-102.25)<0.2
                    and TOMM((e-s).EuclideanNorm())<1.2):
                board.Remove(t); removed_t+=1; break
# B7 GND ball -> nRF exposed pad (both GND, 0.35 mm apart): direct link
if gnd_net is not None:
    _t=pcbnew.PCB_TRACK(board)
    _t.SetStart(V(155.50,102.25)); _t.SetEnd(V(155.50,102.80))
    _t.SetLayer(pcbnew.F_Cu); _t.SetWidth(MM(0.15)); _t.SetNet(gnd_net)
    board.Add(_t)
print(f"pre-clean: removed {removed_v} antenna vias, {removed_t} B7 spokes; B7->EP link added", flush=True)

# save + reload: Remove() leaves stale connectivity that segfaults ZONE_FILLER
# (zone_by_layer / pour_at are defined below and bind to the reloaded board)
board.Save(PCB)
board = pcbnew.LoadBoard(PCB)
filler = pcbnew.ZONE_FILLER(board)
zone_by_layer={}
