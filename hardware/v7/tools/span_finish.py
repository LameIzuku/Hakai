...
def build_local(box):
    ...
    for t in board.GetTracks():
        ...
        if t.GetClass()=="PCB_VIA":
            ...
            for lay in (FC,BC): stamp_disk_local(lay,TOMM(p.x),TOMM(p.y),VIA_D/2+0.02,net)
        else:
            s,e=t.GetStart(),t.GetEnd()
            stamp_seg_local(int(t.GetLayer()),TOMM(s.x),TOMM(s.y),TOMM(e.x),TOMM(e.y),
                            TOMM(t.GetWidth())/2+0.02,net)
    for fp in board.GetFootprints():
        for p in fp.Pads():
            ...
            hx,hy=TOMM(sz.x)/2+0.02,TOMM(sz.y)/2+0.02
            for lay in (FC,BC):
    ...
    def ok(lay,i,j,half):
        ...
        for di in range(-rc,rc+1):
            for dj in range(-rc,rc+1):
                ...
                if math.hypot(di,dj)*G < half+req(o,x,y)+0.03: return False
        return True
...
# ---- 3+4. islands back to ALWAYS, refill, save, DRC --------------------------
