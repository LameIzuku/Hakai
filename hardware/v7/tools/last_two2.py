...
# ---- 1. REVERT the bad horizontal shifts -------------------------------------
moves=[(108.70,109.10),(109.05,109.55)]
sh=0
for t in board.GetTracks():
...
print(f"reverted endpoints: {sh}", flush=True)
...
# ---- 2. C12 tongue: via inside tongue + B loop to the via cluster ------------
c12=try_plans("GND",0.10,[
 [("via",148.00,108.65),
  ("seg",BC,148.00,108.65,148.00,107.00),("seg",BC,148.00,107.00,146.30,107.00),
  ("seg",BC,146.30,107.00,146.30,110.35),("seg",BC,146.30,110.35,146.80,110.35)],
 [("via",148.00,108.65),
  ("seg",BC,148.00,108.65,148.00,107.10),("seg",BC,148.00,107.10,146.25,107.10),
  ("seg",BC,146.25,107.10,146.25,110.40),("seg",BC,146.25,110.40,146.80,110.40)],
])
# ---- 3. C19 tongue: via inside tongue + short B hop to the via group ---------
c19=try_plans("GND",0.10,[
 [("via",152.00,117.70),("seg",BC,152.00,117.70,151.92,116.95)],
 [("via",152.10,117.75),("seg",BC,152.10,117.75,151.95,117.00)],
 [("via",151.80,117.90),("seg",BC,151.80,117.90,151.90,117.05)],
])
