...
# ---- 1. shift the STAT B horizontal y=108.60 -> 108.30 (via-aware follow) ----
sh=0
for t in board.GetTracks():
    if t.GetClass()=="PCB_VIA" or t.GetNetname()!="STAT": continue
    if int(t.GetLayer())!=BC: continue
    s,e=t.GetStart(),t.GetEnd()
    sx,sy,ex,ey=TOMM(s.x),TOMM(s.y),TOMM(e.x),TOMM(e.y)
    if False and abs(sy-108.60)<0.02 and abs(ey-108.60)<0.02 and abs(sx-ex)>5:
        t.SetStart(V(sx,108.30)); t.SetEnd(V(ex,108.30)); sh+=1
        endpoints=[(sx,108.60),(ex,108.60)]
        # follow: any STAT item touching the OLD endpoints gets stretched
        for t2 in board.GetTracks():
            if t2 is t or t2.GetNetname()!="STAT": continue
            if t2.GetClass()=="PCB_VIA":
                p=t2.GetPosition()
                for ox,oy in endpoints:
                    if near(TOMM(p.x),TOMM(p.y),ox,oy):
                        # bridge via to the new line with a stub
                        nt=pcbnew.PCB_TRACK(board)
                        nt.SetStart(V(TOMM(p.x),TOMM(p.y))); nt.SetEnd(V(TOMM(p.x),108.30))
                        nt.SetLayer(pcbnew.B_Cu); nt.SetWidth(MM(0.10)); nt.SetNet(t2.GetNet())
                        board.Add(nt); sh+=1
            else:
                s2,e2=t2.GetStart(),t2.GetEnd()
                for ox,oy in endpoints:
                    if near(TOMM(s2.x),TOMM(s2.y),ox,oy): t2.SetStart(V(TOMM(s2.x),108.30)); sh+=1
                    if near(TOMM(e2.x),TOMM(e2.y),ox,oy): t2.SetEnd(V(TOMM(e2.x),108.30)); sh+=1
print(f"STAT shift ops: {sh}", flush=True)
...
c12=try_plans("GND",0.10,[
 [("via",148.00,108.66),
  ("seg",BC,148.00,108.66,146.30,108.66),
  ("seg",BC,146.30,108.66,146.30,110.35),("seg",BC,146.30,110.35,146.80,110.35)],
 [("via",148.00,108.64),
  ("seg",BC,148.00,108.64,146.25,108.64),
  ("seg",BC,146.25,108.64,146.25,110.40),("seg",BC,146.25,110.40,146.80,110.40)],
])
c19=try_plans("GND",0.10,[
 [("via",152.75,118.60),("seg",BC,152.75,118.60,153.45,119.30),
  ("seg",BC,153.45,119.30,153.45,122.30)],
 [("via",152.60,118.65),("seg",BC,152.60,118.65,153.42,119.45),
  ("seg",BC,153.42,119.45,153.42,122.30)],
 [("via",152.50,118.60),("seg",BC,152.50,118.60,153.40,119.50),
  ("seg",BC,153.40,119.50,153.40,122.30)],
])
