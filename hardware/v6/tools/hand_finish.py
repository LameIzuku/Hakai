...
# ---- 1. VBAT_SENSE shift (frees the x=156.85 exit lane for VBAT_EN) ---------
def near(a,b,x,y,eps=0.03): return abs(a-x)<eps and abs(b-y)<eps
rm=0
for t in list(board.GetTracks()):
    if t.GetClass()=="PCB_VIA" or t.GetNetname()!="VBAT_SENSE": continue
    s,e=t.GetStart(),t.GetEnd()
    sx,sy,ex,ey=TOMM(s.x),TOMM(s.y),TOMM(e.x),TOMM(e.y)
    if int(t.GetLayer())==FC and (near(sx,sy,156.75,99.7936) or near(ex,ey,156.75,99.7936)):
        if max(sy,ey)>101.0:
            board.Remove(t); rm+=1
print(f"VBAT_SENSE vert ripped: {rm}", flush=True)
# purge from the geometric model (seg tuple: 0=tag 1=lay 2..5=xyxy 6=hw 7=net)
items[:]=[it for it in items if not (it and it[0]=="seg" and len(it)>7 and it[7]=="VBAT_SENSE"
          and abs(it[2]-156.75)<0.05 and abs(it[4]-156.75)<0.05
          and 99.5<min(it[3],it[5]) and max(it[3],it[5])<102.0)]
for (x1,y1,x2,y2) in [(156.75,101.75,156.75,101.45),(156.75,101.45,156.55,101.45),
                      (156.55,101.45,156.55,99.95),(156.55,99.95,156.75,99.79)]:
    wr=seg_clear(FC,x1,y1,x2,y2,0.075,"VBAT_SENSE")
    print(f"  VBAT_SENSE leg ({x1},{y1})->({x2},{y2}):","clear" if wr is None else f"BLOCKED {str(wr[2])[:50]} d={wr[0]:.3f}")
    add_seg(x1,y1,x2,y2,FC,"VBAT_SENSE",0.15)
print("VBAT_SENSE re-laid", flush=True)

# ---- 2. STAT micro-bridge -----------------------------------------------------
add_seg(156.00,107.75,156.00,107.90,FC,"STAT",0.10)
print("STAT bridge laid", flush=True)

# ---- 3. the three routes (hand-verified lanes; checker adjudicates) ----------
def try_plans(net,w,plans):
    for pi,plan in enumerate(plans):
        ok=True
        for step in plan:
            if step[0]=="via":
                wr=via_clear(step[1],step[2],net)
            else:
                wr=seg_clear(step[1],step[2],step[3],step[4],step[5],w/2,net)
            if wr is not None:
                print(f"  {net} plan{pi} {step}.. blocked: {str(wr[2])[:55]} d={wr[0]:.3f}<{wr[1]:.3f}")
                ok=False; break
        if ok:
            for step in plan:
                if step[0]=="via": add_via(step[1],step[2],net)
                else: add_seg(step[2],step[3],step[4],step[5],step[1],net,w)
            print(f"  {net}: plan{pi} committed")
            return True
    return False
def S(lay,x1,y1,x2,y2): return ("seg",lay,x1,y1,x2,y2)
def VIA(x,y): return ("via",x,y)

ok={}
ok["BTN_SIDE_BACK"]=try_plans("BTN_SIDE_BACK",0.15,[
 [S(FC,153.75,104.75,152.90,104.75),S(FC,152.90,104.75,152.90,105.90),
  S(FC,152.90,105.90,152.40,105.90),VIA(152.40,105.90),
  S(BC,152.40,105.90,152.40,104.706)],
 [S(FC,153.75,104.75,152.90,104.75),S(FC,152.90,104.75,152.90,106.30),
  S(FC,152.90,106.30,152.40,106.30),VIA(152.40,106.30),
  S(BC,152.40,106.30,152.40,104.706)]])

ok["BTN_RIGHT"]=try_plans("BTN_RIGHT",0.10,[
 [S(FC,153.35,104.50,152.65,104.50),S(FC,152.65,104.50,152.65,104.15),
  VIA(152.65,104.15),
  S(BC,152.65,104.15,153.35,104.15),S(BC,153.35,104.15,153.35,99.15),
  VIA(153.35,99.15),S(FC,153.35,99.15,153.35,97.95),VIA(153.35,97.95),
  S(BC,153.35,97.95,153.35,71.20),VIA(153.35,71.20),
  S(FC,153.35,71.20,153.35,70.48),S(FC,153.35,70.48,153.00,70.48)],
 [S(FC,153.35,104.50,152.65,104.50),S(FC,152.65,104.50,152.65,104.15),
  VIA(152.65,104.15),
  S(BC,152.65,104.15,153.50,104.15),S(BC,153.50,104.15,153.50,99.15),
  VIA(153.50,99.15),S(FC,153.50,99.15,153.50,97.95),VIA(153.50,97.95),
  S(BC,153.50,97.95,153.50,71.20),VIA(153.50,71.20),
  S(FC,153.50,71.20,153.50,70.48),S(FC,153.50,70.48,153.00,70.48)]])

ok["VBAT_EN"]=try_plans("VBAT_EN",0.10,[
 [S(FC,157.00,101.30,157.00,101.25),S(FC,157.00,101.25,156.85,101.25),
  S(FC,156.85,101.25,156.85,99.45),S(FC,156.85,99.45,156.90,99.45),
  S(FC,156.90,99.45,156.90,99.30),VIA(156.90,99.30),
  S(BC,156.90,99.30,146.50,99.30),S(BC,146.50,99.30,146.50,102.55),
  VIA(146.50,102.55),S(FC,146.50,102.55,146.50,104.35),VIA(146.50,104.35),
  S(BC,146.50,104.35,146.50,127.50),S(BC,146.50,127.50,139.56,127.50),
  S(BC,139.56,127.50,139.56,128.30),VIA(139.56,128.30),
  S(FC,139.56,128.30,139.56,129.05)],
 [S(FC,157.00,101.30,157.00,101.25),S(FC,157.00,101.25,156.85,101.25),
  S(FC,156.85,101.25,156.85,99.45),S(FC,156.85,99.45,156.90,99.45),
  S(FC,156.90,99.45,156.90,99.30),VIA(156.90,99.30),
  S(BC,156.90,99.30,147.00,99.30),S(BC,147.00,99.30,147.00,102.55),
  VIA(147.00,102.55),S(FC,147.00,102.55,147.00,104.35),VIA(147.00,104.35),
  S(BC,147.00,104.35,147.00,127.50),S(BC,147.00,127.50,139.56,127.50),
  S(BC,139.56,127.50,139.56,128.30),VIA(139.56,128.30),
  S(FC,139.56,128.30,139.56,129.05)]])
print("routes:",ok, flush=True)

# ---- 4. save FIRST, then fill in a fresh load (SWIG-safe order) --------------
board.Save(PCB)
board=pcbnew.LoadBoard(PCB)
...
out=os.path.join(PROJ,"build","drc_hand.json")
...
print(f"HAND DRC: unconnected={len(d.get('unconnected_items',[]))} "
